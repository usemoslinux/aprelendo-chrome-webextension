...
document.addEventListener("click", (e) => {
    if (e.target.classList.contains("button")) {
        chrome.runtime.sendMessage({"lang": e.target.id});
        window.close();
    }
});
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    chrome.storage.sync.get(languages.map(lang => `show_${lang.code}`), (res) => {
        languages.forEach(lang => {
            if (res[`show_${lang.code}`] || typeof res[`show_${lang.code}`] === 'undefined') {
                ...
                element.textContent = chrome.i18n.getMessage(lang.name);
